...
#define MAX_VORONOI_ATTEMPT_NUMBER 450
		...
		void VoronoiSitesGeneratorImpl::uniformlyGenerateSitesInMesh(
			const uint32_t sitesCount,
			float minDistance) // new parameter: minimal distance between sites
		{
			...
			// This constant sets how many times we try to place a new site before giving up.
			// If minDistance is large, you might fail to place many points.
			static const uint32_t MAX_ATTEMPTS = 5000;
			...
			mGeneratedSites.clear();
			mGeneratedSites.reserve(sitesCount);

			while (generatedSites < sitesCount && attemptNumber < MAX_ATTEMPTS)
			{
				// Generate a candidate within the bounding box:
				float rn1 = mRnd->getRandomValue() * vc.x;
				float rn2 = mRnd->getRandomValue() * vc.y;
				float rn3 = mRnd->getRandomValue() * vc.z;

				NvcVec3 candidate = {rn1, rn2, rn3};
				candidate = candidate + mn; // shift into actual bounding box

				// 1. Check if inside main mesh
				if (!voronoiMeshEval.isPointContainedInMesh(mMesh, candidate))
				{
					attemptNumber++;
					continue;
				}

				// 2. Check if inside stencil (if any)
				if (mStencil != nullptr &&
					!voronoiMeshEval.isPointContainedInMesh(mStencil, candidate))
				{
					attemptNumber++;
					continue;
				}

				// 3. Check distance to already placed sites to enforce "minDistance"
				bool tooClose = false;
				for (const auto &existing : mGeneratedSites)
				{
					NvcVec3 diff = candidate - existing;
				...
				if (tooClose)
				{
					// This candidate is too close to an existing site; try again.
					attemptNumber++;
					continue;
				}

				// If we get here, the candidate is valid�store it:
				mGeneratedSites.push_back(candidate);
				generatedSites++;
				attemptNumber = 0; // reset attempts because we successfully placed a site
			}

			// At this point, you either have "sitesCount" points or hit MAX_ATTEMPTS.
			// The resulting distribution will have a rough minimal spacing of minDistance.
		}
		...
		//	// Define grid spacing
		//	float gridSpacing = cbrt((vc.x * vc.y * vc.z) / sitesCount);
		//	uint32_t generatedSites = 0;

		//	// Loop through the grid points
		//	for (float x = mn.x; x <= mx.x; x += gridSpacing)
		//	{
		//		for (float y = mn.y; y <= mx.y; y += gridSpacing)
		//		{
		//			for (float z = mn.z; z <= mx.z; z += gridSpacing)
		//			{
		//				float rn1 = mRnd->getRandomValue();
		//				float rn2 = mRnd->getRandomValue();
		//				float rn3 = mRnd->getRandomValue();

		//				NvcVec3 point{x + rn1, y + rn2, z + rn3};
		//				// Check if the point is within the mesh (and stencil if provided)
		//				if (voronoiMeshEval.isPointContainedInMesh(mMesh, point) &&
		//					(mStencil == nullptr || voronoiMeshEval.isPointContainedInMesh(mStencil, point)))
		//				{
		//					mGeneratedSites.push_back(point);
		//					generatedSites++;

		//					// Stop if we've generated enough sites
		//					if (generatedSites >= sitesCount)
		//						break;
		//				}
		//			}
		//			if (generatedSites >= sitesCount)
			...
			else
			{
				...
				for (uint32_t chunkIndex = 0; chunkIndex < mChunkData.size(); ++chunkIndex)
				{
					...
					do
					{
						...
						{
							std::for_each(treeWalk.begin(), treeWalk.end(), [&chunkFlags](int32_t index)
										  { chunkFlags[index] |= Mergeable; });
							break;
						}
